...
# -------------------- 代码说明 --------------------
# 本脚本是 #1开始训练_完善版.py 的跨线路增强版，核心改进：
#
#   将同一场站内所有线路的历史数据（含目标线路本身）同时作为特征输入，
#   以显式利用线路间的相关性来提升预测精度。
#
# 与 #1完善版 的唯一差异在特征构造：
#   - #1完善版：每条线路独立建模，仅用该线路自身的历史数据作特征。
...
#
# 完善版同步的所有改进（与 #1完善版 完全一致）：
#   1. GPU 自动检测与回退（LightGBM、XGBoost、LSTM）
#   2. 数据合法性验证（NaN/inf 检测，空样本保护）
#   3. 特征构造时逐行过滤非有限值样本
#   4. Ridge / MLP 流水线增加 SimpleImputer
#   5. LSTM 全面增强：hidden_size=128，AdamW+weight_decay，
#      ReduceLROnPlateau，AMP 混合精度，多 worker DataLoader，
#      完整 loss 合法性校验，早停 patience=15
#   6. 预测长度与有限性校验
#   7. RMSE 兼容不同 sklearn 版本的计算方式
#   8. 单实验级别异常捕获，跳过而不中断整个模型
#   9. 数据加载后基础统计打印
#   10. 结果 dict 增加 device 字段
#
# 支持的模型类型（MODEL_TYPE）：
...
def try_train_lightgbm_gpu(X_tr, y_tr, X_val, y_val):
    ...
    evals_result = {}
    model = lgb.train(
        params_gpu, train_data,
        num_boost_round=2000,
        valid_sets=[train_data, valid_data],
        valid_names=['train', 'valid'],
        callbacks=[early_stopping(stopping_rounds=100), log_evaluation(50), record_evaluation(evals_result)],
    )
