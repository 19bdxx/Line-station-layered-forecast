...
DATA_PATH = "DATA/all_stations_输电线未发电置零_after20240718_15min_mean.csv"
predict_steps = [1, 4, 8, 12, 16]  # 15min, 1h, 2h, 3h, 4h
    ...
    try:
        for station_name, config in stations.items():
            for include_limit in [True, False]:
                sub_dir = os.path.join('targeted_horizons', '对比实验' if include_limit else '对比实验_无限电')
                ...
                for target_col in config['target_cols']:
                    for step in predict_steps:
                        try:
                            ...
                            file_prefix = f"{target_col}_t+{step}"
                            out_csv = os.path.join(save_dir, f"{file_prefix}.csv")
        ...
        summary_df.to_csv(os.path.join('targeted_horizons', f'所有实验汇总_metrics_{model_type}.csv'), index=False)
...
all_summary_df.to_csv(os.path.join('targeted_horizons', '所有实验汇总_metrics_ALL.csv'), index=False)
print(f"\n🎉 全部模型实验完成！综合汇总已保存为 targeted_horizons/所有实验汇总_metrics_ALL.csv")