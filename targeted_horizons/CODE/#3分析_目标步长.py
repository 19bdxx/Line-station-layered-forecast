...
for model_type in MODEL_TYPES:
    ...
    try:
        base_dirs = {
            "with_limit": os.path.join("targeted_horizons", "合并结果", model_type, "with_limit"),
            "no_limit":   os.path.join("targeted_horizons", "合并结果", model_type, "no_limit")
        }
        ...
        if df_result.empty:
            ...
            out_file = os.path.join("targeted_horizons", f"比较分析结果_全站预测加和_vs_直接预测_{model_type}.csv")
...
if df_all_result.empty:
    ...
    df_all_result.to_csv(os.path.join("targeted_horizons", "比较分析结果_全站预测加和_vs_直接预测_ALL.csv"), index=False)
    print("\n🎉 所有模型分析完成！综合结果已保存为：targeted_horizons/比较分析结果_全站预测加和_vs_直接预测_ALL.csv")
