...
for model_type in MODEL_TYPES:
    ...
    try:
        base_dirs = {
            "with_limit": os.path.join("targeted_horizons", "对比实验", model_type),
            "no_limit":   os.path.join("targeted_horizons", "对比实验_无限电", model_type)
        }
        ...
        for limit_mode, base_dir in base_dirs.items():
            ...
            for station, targets in stations.items():
                ...
                # 输出每个预测步长的合并结果
                save_dir = os.path.join("targeted_horizons", "合并结果", model_type, limit_mode, station)
