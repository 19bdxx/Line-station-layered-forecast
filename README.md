...
- **若某类模型在训练/预测过程中抛出异常，会打印 `❌` 错误提示及完整堆栈信息，然后自动跳过该模型，继续执行后续模型，不影响整体流程**
...
- 将合并后文件保存至 `合并结果/` 目录
- **若某类模型的合并过程中抛出异常，会打印 `❌` 错误提示及完整堆栈信息，然后自动跳过该模型，继续处理后续模型**
...
- 将对比结果汇总为一个 CSV 文件
- **若某类模型的分析过程中抛出异常，会打印 `❌` 错误提示及完整堆栈信息，然后自动跳过该模型，继续处理后续模型**
...
| `❌ 模型 [xxx] 执行出错，已跳过` | 该模型依赖未安装或运行时异常（如 LSTM 缺少 PyTorch） | 查看打印的完整堆栈信息定位问题；如暂时不需要该模型，可将其从 `MODEL_TYPES` 列表中注释掉 |
